...
void ADC_init(PinName_t pinName){  // trigmode 0 : SW, 1 : TRGO
	...
	ADC1->SQR3 |= ((chN&0x1F)<<0); 				// Choose the first channelID to sample
...
void JADC_init(PinName_t pinName){
	...
	ADC1->JSQR &= ~(3<<20); 			// 00: conversion in the inject channel

	// Configure the injected channel sequence 
	ADC1->JSQR &= ~ADC_JSQR_JSQ4;			// SQ4 clear bits
	ADC1->JSQR |= ((chN&0x1F)<<15); 			// Choose the channel to convert firstly
	...
	JADC_trigger(TIM5, 1, RISE_ADC);//if tim 3 not change
}
...
void JADC_trigger(TIM_TypeDef* TIMx, int msec, int edge){
	...
	else if((TIMx==TIM4)||(TIMx==TIM5))ADC1->CR2 |= (timer*2+1)<<16; 							// TIMx TRGO event (JADC : TIM1, TIM2, TIM4, TIM5 TRGO)
