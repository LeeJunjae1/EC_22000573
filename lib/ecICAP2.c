...
// 1. Select Timer channel(TIx) for Input Capture channel(ICx)
	...
	TIMx->CCMR1 |= 	(1<<8);  				     	//01<<8   CC2s    TI2=IC2
	TIMx->CCMR2 |= 	(1<<0);        				//01<<0   CC3s    TI3=IC3
	TIMx->CCMR2 |= 	(1<<8);  							//01<<8   CC4s    TI4=IC4
...
// 1. Disable  CC. Disable CCInterrupt for ICn. 
	TIMx->CCER&= ~(0b1010 << 4*(ICn - 1));														// Capture Disable
...
// 2. Configure  IC number(user selected) with given IC pin(TIMx_CHn)
	switch(ICn){
			case 1:
					...
					break;
			case 2:
					TIMx->CCMR1&=~(0b11<<8);										//reset   CC2S
					if (ICn==CHn) TIMx->CCMR1|=(1<<8);     	//01<<0   CC2S    Tx_Ch2=IC2
					else TIMx->CCMR1|=(2<<8);     											//10<<0   CC2S    Tx_Ch1=IC2
					break;
			case 3:
					TIMx->CCMR2 &= ~TIM_CCMR2_CC3S;											//reset   CC3S
					if (ICn==CHn) TIMx->CCMR2|=(1<<0);	    //01<<8   CC3S    Tx_Ch3=IC3
					else TIMx->CCMR2|=(2<<0);		     				//10<<8   CC3S    Tx_Ch4=IC3
					break;
			case 4:
					TIMx->CCMR2&=~(0b11<<8);										//reset   CC4S
					if (ICn==CHn) TIMx->CCMR2|=(1<<8);	   						  //01<<8   CC4S    Tx_Ch4=IC4
					else TIMx->CCMR2|=(2<<8);	     					//10<<8   CC4S    Tx_Ch3=IC4
					break;
			default: break;
		}
